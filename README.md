...
The suite currently stands at 480+ passing tests. Raw input schemas for both the ADS-B timeseries and the QAR truth data are defined, and `tdz.io.readers` reads the delivered files directly: CSV/Parquet loading with schema validation, per-flight `FlightRecord` assembly (with explicit skip reasons), the runway-supplement join for the geometry the files lack (heading, elevation+datum, width — see below), and the QAR-by-airframe match producing truth records. **The remaining work is empirical, not structural:** obtain a runway supplement table, confirm the flagged unit assumptions, run the milestone report on a real QAR slice to characterize the cadence-limited error floor, and ratify the provisional accuracy targets.

Note on the runway supplement: the delivered ADS-B files carry the threshold lat/long and runway length, but **not** the runway heading, threshold elevation (+datum), or width that `RunwayReference` requires. `read_runway_supplement` loads a per-(airport, runway) table supplying those; flights without a supplement entry are skipped with an explicit reason, never guessed.

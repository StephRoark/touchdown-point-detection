...
- [ ] 1. Set up the project skeleton and tooling
...
- [ ] 2. Implement the data models (dataclasses)
...
- [ ] 3. Build the configuration loader, schema, and validation
  - [ ] 3.1 Implement YAML config loading matching the design Configuration Schema (pipeline, timebase, signals, estimators, fusion, quality_gates, lever_arms, geodesy, vertical_crossing, sources, validation, output).
  - [ ] 3.2 Implement schema validation that rejects wrong types, out-of-range values, and unknown estimator names at startup with a specific error naming the parameter and violated constraint; reject a lever-arm table with no entries.
  - [ ] 3.3 Implement defaults resolution: missing parameters fall back to a defaults section and log a warning naming the parameter and applied default; record the fully resolved config for reproducibility.
  - [ ] 3.4 Write property test P12 (random invalid mutations → rejected with correct message) and unit tests for defaults/warning behavior.
  - _Requirements: 20.1, 20.2, 20.3, 20.4, 20.5; Property 12_

### Stage 1 — Geometry, datum, and mapping (test-first)

- [ ] 4. Implement geodesy and runway centerline projection (`tdz.geo`)
  - [ ] 4.1 Implement threshold-relative projection using geodesic / local tangent plane (ENU) math via `pyproj`, returning along-runway distance and lateral offset; never use naive Euclidean on raw lat/long. Use the landing/displaced threshold as origin with the signed convention (positive = past threshold in landing direction).
  - [ ] 4.2 Write property test P1 (round-trip within 0.1 m for distance 0–6000 m, offset ±50 m) and a high-latitude geodesic-vs-Euclidean known-answer test.
  - [ ] 4.3 Implement runway-reference validation rejecting missing/null/out-of-bounds fields (lat ±90, lon ±180, heading 0–360, elevation −500..10000 m, length 0..6000 m, width 0..100 m) with a field-identifying error, and emit `INVALID_RUNWAY_REF`.
  - [ ] 4.4 Write property test P18 (random invalid runway inputs → rejected) and edge tests (heading 0°/360° wrap, distance exactly 0 at threshold).
